...
async function setupLlmKey(
  ...
  if (existingKey) {
    console.log('');
    console.log(
      styleText('green', 'Semantic search') + ' — remote LLM key found. Ready.',
    );
    return;
  }
  ...
  console.log(
    '  relevant documentation by meaning, not just keywords. It works ' +
      styleText('green', 'out of the box'),
  );
  console.log(
    '  with a local embedding model (downloaded on first search, no token needed).',
  );
  console.log(
    '  Optionally, set a remote embedding key (OpenAI or Vercel AI Gateway) for',
  );
  ...
  if (!rl) {
    console.log(
      styleText('dim', '  Using the local embedding model.') +
