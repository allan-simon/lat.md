...
import { getSection, type SectionFound } from './section.js';
import {
  renderMarkdown,
  escapeHtml,
  isSourceTarget,
 ...
 */
function buildResolver(allSections: Section[]): WikiLinkResolver {
...
function indexBody(allSections: Section[]): string {
  ...
  for (const [file, sections] of [...byFile.entries()].sort()) {
    const items = sections
      .map((s) => {
        ...
        return `<li>${escapeHtml(indent)}<a href="/section?id=${encodeURIComponent(
          s.id,
...
async function sectionBody(
    ...
    const items = incomingRefs
      .map((m) => {
        const desc = m.section.firstParagraph
          ...
          : '';
        return `<li><a href="/section?id=${encodeURIComponent(
          m.section.id,
        )}">${escapeHtml(m.section.id)}</a>${desc}</li>`;
      })
      .join('');
    parts.push(
  ...
  if (outgoingRefs.length) {
    const items = outgoingRefs
      .map(
        (r) =>
          `<li><a href="/section?id=${encodeURIComponent(
            r.resolved.id,
          )}">${escapeHtml(r.resolved.id)}</a></li>`,
      )
      .join('');
    parts.push(
        ...
        if (found.kind !== 'found') {
          res
            ...
            .end(
              page(
                'Not found',
                `<p>No section <code>${escapeHtml(id)}</code>.</p>`,
              ),
            );
          return;
        }
