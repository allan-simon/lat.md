...
export type Repo = { base: string; ref: string };

/** Split a `[[src/foo.ts#bar]]`-style target into its file part and symbol path. */
export function splitSourceTarget(target: string): {
  file: string;
  symbol: string;
} {
...
export function isSourceFileTarget(target: string): boolean {
 ...
 */
export function detectRepo(projectRoot: string): Repo | null {
...
export function blobUrl(repo: Repo, file: string, line?: number): string {
 ...
 */
export async function buildSourceLineMap(
  latDir: string,
  projectRoot: string,
  ...
  for (const file of await listLatticeFiles(latDir)) {
    ...
    for (const ref of extractRefs(file, fc, projectRoot)) {
      if (isSourceFileTarget(ref.target)) targets.add(ref.target);
    }
  }

  const lineByTarget = new Map<string, number>();
