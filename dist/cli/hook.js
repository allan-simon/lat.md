...
async function getStopStatus(latDir) {
...
function formatStopReason({ checkFailed, totalErrors, needsSync, codeLines, latMdLines, }) {
...
async function handleClaudeStop() {
    ...
    if (stopHookActive) {
        if (status.checkFailed) {
            console.error(`lat check is still failing (${status.totalErrors} error(s)). Run \`lat check\` to see details.`);
        }
        return;
    }
        ...
        case 'claude':
            switch (event) {
                ...
                case 'Stop':
                    ...
                    return;
                default:
                    console.error(`Unknown hook event for claude: ${event}. Supported: UserPromptSubmit, Stop`);
                    process.exit(1);
            }
        case 'cursor':
            switch (event) {
                case 'stop':
                    ...
                    return;
                default:
                    console.error(`Unknown hook event for cursor: ${event}. Supported: stop`);
                    process.exit(1);
            }
        default:
