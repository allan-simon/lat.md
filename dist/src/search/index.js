 ...
 */
export function chunkWords(text, chunkWords = CHUNK_WORDS, overlap = CHUNK_OVERLAP) {
...
export async function indexSections(latDir, db, provider, key) {
    ...
    if (toEmbed.length > 0) {
        ...
        for (let i = 0; i < toEmbed.length; i++) {
            ...
            const windowVectors = flatVectors.slice(cursor, cursor + windowsPerSection[i].length);
