...
import { flattenSections, buildFileIndex, resolveRef, } from '../lattice.js';
import { renderMarkdown, escapeHtml, isSourceTarget, } from './html.js';
 ...
 */
export function buildResolver(allSections, sectionUrl, sourceHref) {
...
export const STATIC_QUERY_PREFIX = 'Represent this sentence for searching relevant passages: ';
/**
 ...
 */
export function staticSearch(query, docs, queryVec) {
    const tok = (s) => s.toLowerCase().match(/[a-z0-9]+/g) || [];
    const N = docs.length;
    if (!N)
        return [];
    const prepared = docs.map((d) => {
        const terms = tok(`${d.ancestors ? d.ancestors + ' ' : ''}${d.heading} ${d.text}`);
        const tf = {};
    ...
    for (const p of prepared)
        for (const t of Object.keys(p.tf))
            df[t] = (df[t] || 0) + 1;
...
export function buildSidebar(allSections, sectionUrl) {
    ...
    for (const [file, sections] of [...byFile.entries()].sort()) {
        const items = sections
            ...
            return `<li>${escapeHtml(indent)}<a href="${sectionUrl(s.id)}">${escapeHtml(lastSegment(s.id))}</a></li>`;
        })
            .join('');
...
export async function buildSectionContent(found, resolver, sectionUrl, codeRefHref) {
    ...
    if (incomingRefs.length) {
        const items = incomingRefs
            ...
            const desc = m.section.firstParagraph
                ...
                : '';
            return `<li><a href="${sectionUrl(m.section.id)}">${escapeHtml(m.section.id)}</a>${desc}</li>`;
        })
            .join('');
    ...
    if (outgoingRefs.length) {
        const items = outgoingRefs
            .map((r) => `<li><a href="${sectionUrl(r.resolved.id)}">${escapeHtml(r.resolved.id)}</a></li>`)
            .join('');
    ...
    if (codeRefs.length) {
        const items = codeRefs
            ...
            const locHtml = href
                ...
                : loc;
            return (`<li class="coderef">${locHtml}` +
                (c.snippet
...
function stripWiki(s) {
    return s.replace(/\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/g, (_m, target, alias) => alias ? alias : target.split(/[#/]/).pop() || target);
}
/**
 ...
 */
export async function buildIndexContent(latDir, allSections, sectionUrl, graphHref, edges) {
    const resolver = buildResolver(allSections, sectionUrl);
    const parts = ['<h1>lat.md</h1>'];
    ...
    if (hubs.length) {
        const items = hubs
            ...
            const desc = h.section.firstParagraph
                ...
                : '';
            return `<li><a href="${sectionUrl(h.section.id)}">${escapeHtml(lastSegment(h.section.id))}</a>${desc}</li>`;
        })
            .join('');
        parts.push(`<section class="backlinks"><h2>Popular entry points</h2><ul>${items}</ul></section>`);
    }
    parts.push(`<p class="dim">Search above, browse every section in the sidebar, or explore the <a href="${graphHref}">graph view</a>.</p>`);
    return parts.join('\n');
}
