import { openDb, ensureSchema, closeDb, modelFingerprint, commitFingerprint, } from '../search/db.js';
...
async function withDb(latDir, key, progress, fn) {
    ...
    try {
        ...
        if (schema.rebuilt && schema.previousFingerprint) {
            progress?.onModelSwitch?.(schema.previousFingerprint, modelFingerprint(provider));
        }
...
export async function runSearch(latDir, query, key, limit, progress) {
    ...
    try {
        ...
        return keywordFallback(latDir, query, limit, err);
    }
}
/**
 ...
 */
async function keywordFallback(latDir, query, limit, cause) {
    const hits = await keywordSearch(latDir, query, limit);
    const direct = hits.map((h) => ({
        section: h.section,
 ...
 * them.
 */
async function applyGraphExpansion(latDir, direct, limit) {
    const present = new Set(direct.map((m) => m.section.id.toLowerCase()));
    const neighbours = await expandViaGraph(latDir, direct.map((m) => ({ id: m.section.id, score: m.score ?? 0 })));
    const merged = [...direct];
    for (const nb of neighbours) {
        // expandViaGraph already excludes seeds, but guard against duplicates.
        if (present.has(nb.section.id.toLowerCase()))
            continue;
        merged.push({
            section: nb.section,
...
export function cliProgress(reindex, s) {
    return {
        onModelSwitch(from, to) {
            ...
            process.stderr.write(s.dim(`Embedding model changed (${from} → ${to}); rebuilding index from scratch.\n`));
        },
        beforeIndex(isEmpty) {
            if (isEmpty || reindex) {
 ...
 * stays clean.
 */
function noteFallback(ctx, key) {
    if (ctx.mode !== 'cli')
        return;
    const why = key ? 'embedding provider unavailable' : 'no API key configured';
    process.stderr.write(ctx.styler.dim(`Embeddings unavailable (${why}); using keyword fallback. ` +
