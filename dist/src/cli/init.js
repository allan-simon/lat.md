...
import { readAgentsTemplate, readCursorRulesTemplate, readPiExtensionTemplate, readOpenCodePluginTemplate, readSkillTemplate, } from './gen.js';
...
function latHookCommand(style, agent, event) {
...
function cursorHooksTemplate(style) {
    ...
    if (existsSync(gitDir)) {
        try {
            ...
            if (result.trim().length > 0) {
                console.log(styleText('yellow', `  ${entry}`) +
 ...
 */
function hasCodexMcpServer(configPath) {
    if (!existsSync(configPath))
        return false;
    try {
 ...
 * ```
 */
function addCodexMcpServer(configPath, style) {
...
async function writeTemplateFile(root, latDir, relPath, template, genTarget, label, indent, ask) {
    ...
    if (currentHash === templateHash) {
        ...
        console.log(styleText('green', `${indent}${label}`) + ' already up to date');
        return templateHash;
    }
    ...
    console.log(styleText('yellow', `${indent}${label}`) +
 ...
 */
function extractMarkerSection(content) {
 ...
 */
function wrapWithMarkers(template) {
 ...
 */
async function appendTemplateSection(root, latDir, relPath, template, label, indent, ask) {
    const absPath = join(root, relPath);
    const templateHash = contentHash(template);
    ...
    if (existingSection !== null) {
        ...
        if (storedHash && existingSectionHash === storedHash) {
            ...
            const updated = currentContent.slice(0, beginIdx) +
                wrapped +
    ...
    if (storedHash && currentHash === storedHash) {
        ...
        console.log(styleText('green', `${indent}Migrated ${label}`) + ' to marker format');
...
async function writeAgentsSkill(root, latDir, hashes, ask) {
    console.log('');
    console.log(styleText('dim', '  The lat-md skill teaches the agent how to write and maintain lat.md/ files.'));
...
async function setupAgentsMd(root, latDir, template, hashes, ask) {
    const hash = await appendTemplateSection(root, latDir, 'AGENTS.md', template, 'AGENTS.md', '', ask);
    if (hash)
...
async function setupClaudeCode(root, latDir, template, hashes, ask, style) {
    ...
    const hash = await appendTemplateSection(root, latDir, 'CLAUDE.md', template, 'CLAUDE.md', '  ', ask);
    if (hash)
        hashes['CLAUDE.md'] = hash;
    // Hooks — UserPromptSubmit (lat.md reminders + [[ref]] expansion) and Stop (update reminder)
    console.log('');
    console.log(styleText('dim', '  Hooks inject lat.md workflow reminders into every prompt and remind'));
    console.log(styleText('dim', '  the agent to update lat.md/ before finishing.'));
    const claudeDir = join(root, '.claude');
    const settingsPath = join(claudeDir, 'settings.json');
    mkdirSync(claudeDir, { recursive: true });
    syncLatHooks(settingsPath, style);
    console.log(styleText('green', '  Hooks') + ' synced (UserPromptSubmit + Stop)');
    ...
    console.log('');
    console.log(styleText('dim', '  Agents can call `lat` from the command line, but an MCP server gives lat'));
    console.log(styleText('dim', '  more visibility and makes agents more likely to use it proactively.'));
...
async function setupCursor(root, latDir, hashes, ask, style) {
    ...
    // .cursor/hooks.json
    console.log('');
    console.log(styleText('dim', '  Cursor hooks can enforce the lat.md/ stop check, while prompt guidance'));
    console.log(styleText('dim', '  stays in rules + MCP because Cursor cannot reliably inject prompt-specific context.'));
    const hooksHash = await writeTemplateFile(root, latDir, '.cursor/hooks.json', cursorHooksTemplate(style), null, 'Hooks (.cursor/hooks.json)', '  ', ask);
    if (hooksHash)
        hashes['.cursor/hooks.json'] = hooksHash;
    // .cursor/mcp.json
    console.log('');
    console.log(styleText('dim', '  Agents can call `lat` from the command line, but an MCP server gives lat'));
    console.log(styleText('dim', '  more visibility and makes agents more likely to use it proactively.'));
    ...
    if (hasMcpServer(mcpPath, 'mcpServers')) {
        ...
        addMcpServer(mcpPath, 'mcpServers', style);
        console.log(styleText('green', '  MCP server') + ' registered in .cursor/mcp.json');
    }
    ...
    console.log('');
    console.log(styleText('yellow', '  Note:') +
...
async function setupCopilot(root, latDir, hashes, ask, style) {
    ...
    const hash = await appendTemplateSection(root, latDir, '.github/copilot-instructions.md', readAgentsTemplate(), 'Instructions (.github/copilot-instructions.md)', '  ', ask);
    if (hash)
    ...
    if (hasMcpServer(mcpPath, 'servers')) {
        ...
        console.log(styleText('green', '  MCP server') + ' registered in .vscode/mcp.json');
    }
    // .agents/skills/lat-md/SKILL.md — skill for authoring lat.md files
    await writeAgentsSkill(root, latDir, hashes, ask);
}
async function setupPi(root, latDir, hashes, ask, style) {
    ...
    console.log('');
    console.log(styleText('dim', '  The Pi extension registers lat tools and hooks into the agent lifecycle'));
    console.log(styleText('dim', '  to inject search context and validate lat.md/ before finishing.'));
...
async function setupOpenCode(root, latDir, hashes, ask, style) {
    // AGENTS.md — OpenCode reads this natively
    // (already created in the shared step if any non-Claude agent is selected)
    // .opencode/plugins/lat.ts — plugin that registers tools + lifecycle hooks
    console.log('');
    console.log(styleText('dim', '  The OpenCode plugin registers lat tools and hooks into the session'));
    console.log(styleText('dim', '  lifecycle to validate lat.md/ when the agent finishes.'));
    const template = readOpenCodePluginTemplate().replace('__LAT_BIN__', latBinString(style));
    const hash = await writeTemplateFile(root, latDir, '.opencode/plugins/lat.ts', template, 'opencode-plugin.ts', 'Plugin (.opencode/plugins/lat.ts)', '  ', ask);
    if (hash)
        hashes['.opencode/plugins/lat.ts'] = hash;
    // .agents/skills/lat-md/SKILL.md — skill for authoring lat.md files
    await writeAgentsSkill(root, latDir, hashes, ask);
    // Ensure .opencode is gitignored (plugin contains local absolute paths)
    ensureGitignored(root, '.opencode');
}
async function setupCodex(root, latDir, hashes, ask, style) {
    ...
    if (hasCodexMcpServer(mcpPath)) {
        ...
        console.log(styleText('green', '  MCP server') + ' registered in .codex/config.toml');
    }
    ...
    console.log(styleText('dim', '  The lat-md skill teaches the agent how to write and maintain lat.md/ files.'));
    const skillTemplate = readSkillTemplate();
    const skillHash = await writeTemplateFile(root, latDir, '.codex/skills/lat-md/SKILL.md', skillTemplate, 'skill.md', 'Skill (.codex/skills/lat-md/SKILL.md)', '  ', ask);
    if (skillHash)
        hashes['.codex/skills/lat-md/SKILL.md'] = skillHash;
}
// ── LLM key setup ───────────────────────────────────────────────────
async function setupLlmKey(rl) {
    ...
    if (!key) {
        console.log(styleText('dim', '  Skipped.') +
            ' You can set ' +
            styleText('cyan', 'LAT_LLM_KEY') +
            ' later or re-run ' +
            styleText('cyan', 'lat init') +
    ...
    if (key.startsWith('sk-ant-')) {
        console.log(styleText('red', '  That looks like an Anthropic key.') +
    ...
    if (!key.startsWith('sk-') && !key.startsWith('vck_')) {
        console.log(styleText('yellow', '  Unrecognized key prefix.') +
    ...
    console.log(styleText('green', '  Key saved') +
        ' to ' +
        styleText('dim', getConfigPath()));
}
// ── Post-onboarding guidance ─────────────────────────────────────────
const NEXT_STEP_PROMPT = 'Read through this codebase and set up lat.md/ to document its architecture, key design decisions, and domain concepts. Run `lat check` when done.';
function printNextSteps(selectedAgents) {
    ...
    if (!hasClaudeCode && ideAgents.length === 0)
        return;
    console.log('');
    console.log(styleText('bold', 'Next step') +
    ...
    if (ideAgents.length > 0) {
        ...
        console.log('');
        console.log('  ' + styleText('bold', `${names}`) + ' — paste into agent chat:');
...
export async function initCmd(targetDir) {
    ...
    try {
        ...
        const selectedAgents = await checklistMenu(allAgents, 'Which coding agents do you use?');
        ...
        const needsLatCommand = useClaudeCode ||
            usePi ||
            useCursor ||
            useCopilot ||
            useOpenCode ||
        ...
        if (!anySelected) {
            console.log('');
            console.log(styleText('dim', 'No agents selected. You can re-run') +
                ' lat init ' +
                styleText('dim', 'later.'));
            return;
        }
        ...
        const needsAgentsMd = usePi || useCursor || useCopilot || useOpenCode || useCodex;
        if (needsAgentsMd) {
        ...
        if (!(await hasRipgrep())) {
            console.log('');
            console.log(styleText('yellow', 'Tip:') +
                ' Install ' +
                styleText('cyan', 'ripgrep') +
                ' (rg) for faster code scanning.' +
                ' See ' +
                styleText('underline', 'https://github.com/BurntSushi/ripgrep#installation'));
        }
