...
import { openDb, ensureSchema, closeDb, modelFingerprint, commitFingerprint, } from '../src/search/db.js';
import { indexSections, chunkWords, meanPool, CHUNK_WORDS, CHUNK_OVERLAP, } from '../src/search/index.js';
import { searchSections, distanceToScore } from '../src/search/search.js';
import { keywordSearch } from '../src/search/keyword.js';
import { buildFtsMatch, fuseCandidates, lexicalCandidates, hybridSearch, minMaxNormalize, DENSE_WEIGHT, } from '../src/search/hybrid.js';
import { createServer } from 'node:http';
import { existsSync } from 'node:fs';
import { embedLocal, resolveLocalModel, localProvider, modelCacheDir, } from '../src/search/local.js';
import { expandViaGraph, HOP_COUNT, GRAPH_PENALTY, } from '../src/search/graph.js';
...
describe('detectProvider', () => {
    ...
    it('rejects an unknown local model id', () => {
        expect(() => detectProvider('local:does-not-exist')).toThrow(/Unknown local embedding model/);
    });
    it('gives the local provider a distinct model fingerprint', () => {
        expect(modelFingerprint(detectProvider('local:qwen3-0.6b'))).toBe('local:Qwen3-Embedding-0.6B-Q8_0:1024');
    });
});
...
describe('ensureSchema model fingerprint', () => {
    ...
    it('writes the fingerprint on a fresh DB without rebuilding', async () => {
        ...
        try {
            ...
            const meta = await db.execute("SELECT value FROM meta WHERE key = 'embedding_fingerprint'");
    ...
    it('drops and rebuilds the index when the model fingerprint changes, committing the new fingerprint only after a successful re-embed', async () => {
        ...
        try {
            ...
            const metaBeforeCommit = await db.execute("SELECT value FROM meta WHERE key = 'embedding_fingerprint'");
            ...
            const metaAfterCommit = await db.execute("SELECT value FROM meta WHERE key = 'embedding_fingerprint'");
...
describe('keywordSearch', () => {
    function makeLat() {
        ...
        const latDir = join(tmp, 'lat.md');
        mkdirSync(latDir, { recursive: true });
        writeFileSync(join(latDir, 'a.md'), '# Authentication\n\nHow we verify user logins and security tokens.\n\n' +
    ...
    it('ranks heading-overlapping sections first with a bounded score', async () => {
        ...
        try {
            const hits = await keywordSearch(latDir, 'authentication and security', 5);
...
describe('expandViaGraph', () => {
    function makeChain() {
        ...
        const latDir = join(tmp, 'lat.md');
        mkdirSync(latDir, { recursive: true });
        writeFileSync(join(latDir, 'foo.md'), '# Foo\n\nLeading. See [[foo#Bar]].\n\n' +
            '# Bar\n\nLeading. See [[foo#Baz]].\n\n' +
            '# Baz\n\nLeading para.\n');
        return { tmp, latDir };
    }
...
describe.skipIf(!localModelCached)('embedLocal (Qwen3-0.6B GGUF)', () => {
    ...
    it('ranks a paraphrase above an unrelated text (query gets the instruct prefix)', async () => {
        ...
        const [query] = await embedLocal(['where is the configuration directory located'], provider, true);
...
describe('buildFtsMatch', () => {
    it('wraps each alphanumeric token in quotes, OR-joined', () => {
        expect(buildFtsMatch('getConfigDir resolution')).toBe('"getconfigdir" OR "resolution"');
    });
    it('strips FTS5 operator characters so a query can never be a syntax error', () => {
        ...
        expect(buildFtsMatch('foo#Bar-baz: (NEAR)')).toBe('"foo" OR "bar" OR "baz" OR "near"');
    });
...
describe('minMaxNormalize', () => {
    it('maps equal scores all to 1 instead of dividing by a zero span', () => {
        const out = minMaxNormalize(new Map([
            ['a', 3],
            ['b', 3],
        ]));
...
describe('hybrid search surfaces an exact identifier the dense model under-ranks', () => {
    ...
    function vectorFor(text) {
        ...
        if (text.includes('configuration directory'))
            return sparse(0); // Config dir
        if (text.includes('getConfigDir helper'))
            return sparse(1); // Resolver (identifier)
        if (text.includes('deployment and servers'))
            return sparse(2); // Unrelated
    ...
    it('lexical (FTS5) finds the identifier section; fusion lifts it above the dense-last rank', async () => {
        ...
        try {
            ...
            expect(byId.get('lat.md/x#Config dir')).toBeGreaterThan(byId.get('lat.md/x#Resolver'));
            expect(byId.get('lat.md/x#Unrelated')).toBeLessThan(byId.get('lat.md/x#Resolver'));
        }
        finally {
            await closeDb(db);
            rmSync(tmp, { recursive: true, force: true });
...
describe.skipIf(!canRun)('search (rag)', () => {
    ...
    it('finds auth section for login query', async () => {
        const results = await searchSections(db, 'how do we handle user login and security?', provider, replayKey);
    ...
    it('finds performance section for latency query', async () => {
        const results = await searchSections(db, 'what tools do we use to measure response times?', provider, replayKey);
        expect(results.length).toBeGreaterThan(0);
