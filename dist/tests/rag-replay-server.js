...
// --- Replay mode ---
function readVector(buf, index, dimensions) {
    const vec = [];
...
function createReplayHandler(replayDir) {
    const manifest = JSON.parse(readFileSync(join(replayDir, 'manifest.json'), 'utf-8'));
...
// --- Capture mode ---
function createCaptureHandler(replayDir, realProvider, realKey) {
    ...
    const flush = () => {
        ...
        const manifest = { dimensions, vectors: {} };
...
// --- Public API ---
export function startReplayServer(replayDir, opts) {
    let handler;
    let flush = () => { };
    if (opts?.capture) {
...
export function hasReplayData(replayDir) {
