        ...
        if (node.type === 'function_definition') {
            ...
            for (const child of node.namedChildren) {
                if (child.type === 'enum_specifier') {
                    collectCEnumMembers(child, symbols, name ?? extractName(child) ?? undefined);
                }
 ...
 */
function cFieldName(declarator) {
    let node = declarator;
 ...
 */
function phpConstName(constElement) {
 ...
 */
function extractPhpMembers(body, ownerName, symbols) {
    for (let i = 0; i < body.namedChildCount; i++) {
        const member = body.namedChild(i);
 ...
 */
function collectPhpNodes(parent, symbols) {
    for (let i = 0; i < parent.childCount; i++) {
        const node = parent.child(i);
        const startLine = node.startPosition.row + 1;
        const endLine = node.endPosition.row + 1;
        if (node.type === 'namespace_definition') {
            ...
            if (name) {
                ...
                const body = node.childForFieldName('body');
                if (body)
                    extractPhpMembers(body, name, symbols);
            }
        }
        else if (node.type === 'const_declaration') {
...
function extractPhpSymbols(tree) {
    const symbols = [];
