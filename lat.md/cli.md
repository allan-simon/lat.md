...
`[[wiki links]]` are resolved through the lattice: section targets become `/section?id=…` anchors; source-symbol targets (`[[src/foo.ts#bar]]`) link to the code on GitHub — a blob URL pinned to the built commit, at the symbol's line — when a repo is detected ([[src/codelink.ts#detectRepo]]), else they render as inert `<code>`. The "Referenced by code" `@lat:` back-refs link the same way. Unresolved targets are flagged with a `broken` class so authors notice. Fenced ` ```mermaid ` blocks become `<pre class="mermaid">` for client-side rendering via the mermaid ESM bundle (loaded from a CDN); other fences keep a `language-*` class. Raw HTML — notably widget `<iframe>`s — passes through via `rehype-raw`.
...
By default this is **BM25 only** (lexical, zero extra weight). With `--dense`, `lat build` also embeds each section at build time with a small model ([[src/render/site.ts#STATIC_EMBED_MODEL]], `Xenova/bge-small-en-v1.5`, 384-dim) via the optional `@huggingface/transformers` dependency, and ships the L2-normalized vectors in the index. The page then lazy-loads the **same** model (~34 MB, from a CDN) to embed the query in-browser — applying the model's query prefix ([[src/render/site.ts#STATIC_QUERY_PREFIX]]), since bge is asymmetric — and `staticSearch` fuses cosine similarity with BM25 (0.75 dense / 0.25 bm25, matching [[src/search/fusion.ts]]).

Both the document embedding and the BM25 text prepend the section's **ancestor heading path** (e.g. `CLI > search`) as situating context — a deterministic form of contextual retrieval. The model and weights were chosen with the relevance harness (`scripts/eval-relevance.ts`); on an independently-authored 90-query gold set this config scored best, though the margins over the prior MiniLM config were small and not statistically significant.
