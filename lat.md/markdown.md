...
Wiki links can reference symbols in TypeScript, JavaScript, Python, Rust, Go, C, and PHP source files:
...
- **`[[src/Service/Foo.php#Foo#bar]]`** — the `bar` method on class `Foo` in PHP
- **`[[src/config.ts]]`** — link to the file itself (no symbol)

Supported extensions: `.ts`, `.tsx`, `.js`, `.jsx`, `.py`, `.rs`, `.go`, `.c`, `.h`, `.php`.
...
PHP symbols: top-level functions, classes, interfaces, traits, enums, methods, class constants, enum cases, and global constants. Members are resolved via the enclosing type — `[[file.php#Class#method]]` matches a `method_declaration` in `class`/`interface`/`trait`/`enum Class`. Enum cases can be referenced standalone (`[[file.php#PENDING]]`) or qualified (`[[file.php#Status#PENDING]]`). Both semicolon (`namespace App;`) and braced (`namespace App { ... }`) namespaces are handled.
